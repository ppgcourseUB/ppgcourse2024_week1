...
# option `print = -1`
cd $i
printf "[[ Generating tree file for concatenated \"mcmc.txt\"  ... ... | "$name_clock" ]]\n\n"
cp $ctl_dir/*ctl .
...
printf "[[ Running MCMCtree with option \"print = -1\" for samples collected under "$name_clock" ]]\n\n"
$mcmcrun *ctl
printf "\n\n[[ Generating tree file with all the collected samples ]]\n"
name_tt_out=$( echo $i | sed 's/mcmc\_files\_//' )
mv FigTree.tre FigTree_$name_tt_out"_95HPD.tree"
printf "[[ Saving output tree file as FigTree_"$name_tt_out"_95HPD.tree ]]\n\n"
cd $base_dir
done
