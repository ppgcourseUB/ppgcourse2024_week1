...
names( calibrations ) <- names( keep_indexes )  <- names( ind_dup ) <- 
  names( nodes_dup ) <- names( tt_all ) <- c( "cytb" )

# Path to tree
path_tree <- c( "../../00_aln_trees_raw/tree/tree_nobl.tree" )
...
for( c in 1:length( calibrations ) ){
  ...
  writeLines( text = paste( length(tt_all[[c]]$tip.label ), " 1", sep = "" ), 
              con = paste( "../../calibs/raw_calibs/cals_only_",
              names( calibrations )[c], ".tree", sep = "" ) )
  ape::write.tree( phy = tt_all[[c]],
                   file = paste( "../../calibs/raw_calibs/cals_only_",
                   names( calibrations )[c], ".tree", sep = "" ),
                   append = TRUE )
}

#>> TEST
tt_all[[1]]$node.label[c(1,3)]
# plot.phylo(tt_all[[c]])
...
tt_name <- c( "../../calibs/raw_calibs/cals_only_cytb.tree" )
