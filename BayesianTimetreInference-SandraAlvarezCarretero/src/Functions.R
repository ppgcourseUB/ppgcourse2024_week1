                              ...
                              multi_calib = FALSE )
{
  ...
  for( k in 1:length(dat) ){
    if( multi_calib == TRUE ){
      calib_nodes <- calib_nodes[[ k ]]
    }else{
      calib_nodes <- calib_nodes
    }
    name_labs   <- calib_nodes[[1]][,2] ## one set of cals per dataset
    labs_in_csv <- paste( "t_n", calib_nodes[[1]][,2], sep = "" )
    ...
    for( i in 1:length(labs_in_csv) ){
      pdf( file = paste( home_dir, "plots/priorVSpost_", dat[k], "_", 
                         calib_nodes[[1]][i,1], "_", name_labs[i],
                         "_plot.pdf", sep = "" ), 
           paper = "a4r", width = 0, height = 0 )
      # Get calibration densities in correct formatting (4 nums)
      is_B    <- grep( x = calib_nodes[[1]][i,3], pattern = "B\\(" )
      is_U    <- grep( x = calib_nodes[[1]][i,3], pattern = "U\\(" )
      is_L    <- grep( x = calib_nodes[[1]][i,3], pattern = "L\\(" )
      is_flag <- grep( x = calib_nodes[[1]][i,3], pattern = "flag" )
      ...
      # in the list
      tmp_name_calib <- calib_nodes[[1]][i,1]
