...
scripts_dir
home_dir <- gsub( pattern = "02_MCMC_diagnostics/scripts",
                  replacement = "01_timetree_inference/", x = scripts_dir )
...
# Record a log file
if( ! dir.exists( paste( out_dir, "out_Rlogs/", sep = "" ) ) ){
  dir.create( paste( out_dir, "out_Rlogs/", sep = "" )  )
}
sink( paste( out_dir, "out_Rlogs/log_MCMCdiagn_margVScalib.txt",
...
# --> When two alignment blocks, there are 2: `mu1`, `mu2`
...
cat( "\n Data ID (one per dataset): ", dat, "\n",
     "Output directory:\n", 
     paste( "\"", out_dir, "plots/margVScalib\"", sep = "" ), "\n\n" )
cat( "\n[[ CALIBRATIONS ]] \n\n" )
calib_nodes[[ 1 ]]
cat( "\n - - - - - - - - - - - - - \n\n" )
...
for( i in c( path_prior) ){
  ...
  mcmc_priors[[count]] <- load_dat( mcmc = paste( i, "mcmc.txt", sep = "" ),
                                    delcol = delcol_prior[count], perc = perc,
...
if( ! dir.exists( paste( out_dir, "plots/margVScalib", sep = "" ) ) ){
  dir.create( paste( out_dir, "plots/margVScalib", sep = "" ) )
}

# Write down how many rows and how many columns
# the plot with all calibrated nodes should have.
# In this case, we have 2 calibrations so we will just have a file with
...
for( i in 1:num_dirs ){
  ...
  plot_check_calibnodes( calibs = calib_nodes[[ 1 ]],
                         ...
                         clock = "CLK", main_wd = out_dir, ind = TRUE,
...
cat( "\n\n ~> All the plots in PDF/JPG/TIFF format have been saved in the",
     "following directory: \n",
     paste( "\"", out_dir, "plots/margVScalib/\"", sep = "" ),
     "\n ~> You will find the plots inside the corresponding subdirectory, one per",
     "dataset: ", dat, "\n" ) 
cat( "\n ~> A log file with the screen output has also been saved as \n",
     paste( "\"",home_dir, "out_Rlogs/log_MCMCdiagn_margVScalib.txt\"", sep = "" ), 
     "\n\n" )
