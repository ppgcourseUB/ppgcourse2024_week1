...
#-------------------------------------------------------------#
# Read trees
## tt_list[[ i ]][[ 1 ]]: tree with node labels as of `MCMCtree`
## tt_list[[ i ]][[ 2 ]]: tree with node labels including `MCMCtree` notation
#                         and node name
# Put them in a list
dat <- c( "cytb" )
...
for( i in 1:length( dat ) ){
  ...
  if( length( dat ) > 1 ){
    tt_list[[ i ]][[ 1 ]] <- ape::read.tree( file = paste( "../../01_timetree_inference/01_MCMCtree/00_prior/",
                                                           "node_tree_", i, ".tree",
                                                           sep = "" ) )  
  }else{
    tt_list[[ i ]][[ 1 ]] <- ape::read.tree( file = paste( "../../01_timetree_inference/01_MCMCtree/00_prior/",
  ...
  tt_list[[ i ]][[ 2 ]] <- ape::read.tree( file = paste( "../../00_inp_data/",
                                                         dat[i],
